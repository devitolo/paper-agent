...
Scout topics live in `config/topics.yaml` and can be steered from `/topics` in the Review Queue web server. The default add/update flow is TopicAgent: describe what you want Project Paper to scout, local Ollama/Qwen proposes structured topic config, and you approve before anything is saved. The default model is `qwen2.5:1.5b-instruct`, configurable with `PAPER_AGENT_TOPIC_MODEL`; the Ollama generate URL defaults to the local Mini runtime and can be changed with `PAPER_AGENT_OLLAMA_URL`. Slow or invalid Qwen responses fall back to a deterministic proposal after `PAPER_AGENT_TOPIC_TIMEOUT_SECONDS`, default 30 seconds. Scheduled source jobs select enabled config topics on future runs, while explicit CLI `--topic` values still override the config for one run. Curator also penalizes obvious physical-world incident domains such as railway, traffic/vehicular, medical/healthcare, power grid, smart grid, and transportation incidents.
...
The review UI reads selected papers from SQLite, shows triage fields, links to registered artifacts, and writes feedback rows. Bind to `0.0.0.0` only on a trusted LAN. It uses compact inbox-style controls, exposes the original paper link with a URL copy control, shows distinct source badges for arXiv/OpenAlex/Semantic Scholar/unknown sources, filters by source when multiple Scout sources are present, keeps quick status actions close to each paper, and shows a clearly labeled source abstract when local triage extraction is missing. Status filters display as All, Scored, Needs review, Interested, Read later, Reviewed, and Not interested. `Scored` means user feedback-backed papers, including papers with structured feedback or raw-only feedback after a parse failure; it can include papers outside the current recommendation queue. `Reviewed` is only the lightweight status option. Quick status buttons are status-only and should complete quickly; they show `Saving...` and a timeout hint if completion hangs. Only saving a non-empty Feedback blob stores the user score/decision and queues Gemini profile apply in the background. Cards label Curator ranking as `System`; when parsed user feedback includes an integer or decimal score, `Your score: N/5` is shown prominently and the system score is visually demoted.
