...
Scout topics are file-backed in `config/topics.yaml` and intentionally cover AIOps, LLM/agentic operations, incident response, root-cause/failure diagnosis, observability/log/trace analysis, debugging, software maintenance, SRE, cloud operations, and production engineering. Curator penalizes obvious physical-world incident domains such as railway, traffic/vehicular, medical/healthcare, grid, and transportation incidents. The web server includes `/topics`, an editable Topic Management V1 page. The fast path accepts one label such as `Datalake operations`, infers a baseline operations query, enables all active sources, and uses daily/normal/enabled defaults. The topic list is collapsed by default and expands into one-line rows; use the row Enable/Disable action for quick toggles, or Edit to open a focused panel for query, sources, cadence, priority, and enabled changes. Duplicate labels or queries are rejected. Changes affect future scheduled runs, not recommendations already created.
