...
- [x] Same-image backup/restore passed the documented Docker drill on Mac Apple Silicon and Ubuntu x86-64; see [backup evidence](package-backup-qa.md). Seven daily/four weekly retention and independent storage are documented; automated scheduling/pruning is deferred. Disk-full/interruption fault tests and visual restored-content verification remain separate open checks.
