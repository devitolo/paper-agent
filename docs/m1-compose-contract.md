...
Ship one versioned application image reference with a multi-platform manifest containing `linux/amd64` and `linux/arm64`. The release Compose file resolves that single reference; do not create separate Mac and Linux product images or force amd64 emulation on Apple Silicon. The selected Ollama image is `docker.io/ollama/ollama@sha256:684d8674b4315fa18f4f0e973a118ec2652ed96f67563277839985175858e0ba`. The Project Paper image is public with both target variants, SBOM, and provenance; released defaults pin the verified index digest. Ubuntu x86-64 passed live installer, model, discovery, persistence, and recovery acceptance on 2026-09-11. macOS Apple Silicon passed the v0.1.3 fresh-install acceptance gate on 2026-09-12.
