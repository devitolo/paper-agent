...
The review UI reads selected papers from SQLite, shows triage fields, keeps artifacts in a collapsed details section, and writes feedback rows. Bind to `0.0.0.0` only on a trusted LAN. It uses compact inbox-style controls, defaults to Highest score, opens the paper from the title, keeps Copy next to source/date metadata, shows distinct source badges for arXiv/OpenAlex/Semantic Scholar/unknown sources, filters by source when multiple Scout sources are present, and shows a clearly labeled source abstract when local triage extraction is missing. Primary filters display as All, Scored, and Needs review. `Scored` means user feedback-backed papers, including papers with structured feedback or raw-only feedback after a parse failure; it can include papers outside the current recommendation queue. Historical lightweight statuses such as interested, read_later, not_interested, and reviewed remain compatible with older CLI/parser paths, but they are no longer primary actions and can be removed from the legacy feedback table with `python3 -m paper_agents.cli db cleanup-legacy-feedback --yes`. Feedback is hidden behind Add feedback or View/edit feedback until needed. Cards label Curator ranking as `Match Score` and surface Curator rationale as `Why this matches you`; when parsed user feedback includes an integer or decimal score, `Your score: N/5` is shown prominently and the match score is visually demoted.
