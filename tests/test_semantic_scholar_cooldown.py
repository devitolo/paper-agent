...
class SemanticScholarCooldownTests(unittest.TestCase):
    def test_exhausted_429_stops_all_topics_and_later_fetches(self):
        source = SemanticScholarSource(retries=1, verbose=False, api_key="test-secret-key")
        error = urllib.error.HTTPError('https://example.test', 429, 'limited', {'Retry-After': '17'}, None)
        with patch('paper_agents.scout.urllib.request.urlopen', side_effect=error) as fetch, patch('paper_agents.scout.time.sleep') as sleep:
        ...
        self.assertNotIn("test-secret-key", json.dumps(source.last_diagnostics))
    ...
    def test_successful_retry_does_not_trigger_cooldown(self):
        ...
        source = SemanticScholarSource(retries=1, request_delay=0, verbose=False)
        error = urllib.error.HTTPError('https://example.test', 429, 'limited', {}, None)
        with patch('paper_agents.scout.urllib.request.urlopen', side_effect=[error, BytesIO(b'{"data": []}'), BytesIO(b'{"data": []}')]) as fetch, patch('paper_agents.scout.time.sleep'):
    ...
    def test_retry_after_http_date_is_respected(self):
        with patch('paper_agents.scout.time.time', return_value=0):
            self.assertEqual(SemanticScholarSource()._retry_delay(0, 'Thu, 01 Jan 1970 00:02:00 GMT'), 120)
