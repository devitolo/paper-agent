...
if mode == 'success':
    ...
    print('{"profile":{"interests":["PRIVATE_PROFILE"]},"change_summary":"ok"}', flush=True)
...
class GeminiProcessTests(unittest.TestCase):
    ...
    def call_fixture(self, mode):
        with patch('paper_agents.feedback.run_gemini', side_effect=lambda command, timeout: run_gemini(self.command(mode), 1)):
    ...
    def test_descendants_cleaned_when_parent_exits_or_ignores_term(self):
        for mode in ('child_pipes', 'resistant', 'closed_pipes'):
        ...
        for sig in (signal.SIGTERM, signal.SIGINT):
            with self.subTest(signal=sig):
                ...
                driver = subprocess.Popen([sys.executable, '-c',
                    'import sys,signal; signal.signal(signal.SIGINT, signal.default_int_handler); from paper_agents.gemini_process import run_gemini; run_gemini(sys.argv[1:], 60)',
                    *self.command('resistant')], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
                try:
    ...
    def test_timeout_preserves_active_profile_and_pending_feedback(self):
        ...
        with patch('paper_agents.feedback.run_gemini', side_effect=lambda command, timeout: run_gemini(self.command('hang'), 1)):
